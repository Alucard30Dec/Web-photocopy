...
public class PrintJobsController : Controller
{
    ...
    public async Task<IActionResult> Create(CreatePrintJobViewModel model, CancellationToken cancellationToken)
    {
        ...
        try
        {
            foreach (var existingId in existingIds)
            {
                ...
                preparedFiles.Add(new PreparedPrintFile(metadata.Id, metadata.OriginalFileName, totalPages.Value));
            }
    ...
    private sealed record PreparedPrintFile(Guid FileId, string FileName, int TotalPages);
