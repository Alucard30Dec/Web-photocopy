...
public sealed class WebPhotocopyHubConnectionStringProvider : IWebPhotocopyHubConnectionStringProvider
{
    ...
    public string Get_Connection_String()
    {
        var v_arrCandidate = new[]
        {
            ...
            _configuration["WEBPHOTOCOPYHUB_POSTGRES_CONNECTION"],
    ...
    {
        var v_strValue = Environment.GetEnvironmentVariable("WEBPHOTOCOPYHUB_ALLOW_SUPABASE_DIRECT");
        if (string.IsNullOrWhiteSpace(v_strValue))
        {
            v_strValue = Environment.GetEnvironmentVariable("PHOTOCOPYHUB_ALLOW_SUPABASE_DIRECT");
        }
