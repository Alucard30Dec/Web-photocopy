(function () {
    ...
    var activePreviewAbortController = null;
    ...
    function createUploadCard(file, index) {
        ...
        previewButton.addEventListener("click", function () {
            showLocalPreview(file, pageInput, previewButton);
        });
    ...
    async function showOfficePreview(file, pageInput, previewButton) {
        if (!officePreviewUrl) {
            showPreviewFallback(
                file.name,
        ...
        if (previewButton) {
            ...
            previewButton.textContent = "Đang chuyển...";
        }

        var requestController = typeof AbortController !== "undefined"
            ? new AbortController()
            : null;
        activePreviewAbortController = requestController;
        ...
        try {
            ...
            if (responseContentType.indexOf("application/pdf") < 0) {
                throw new Error(
                    "Máy chủ không trả về PDF hợp lệ cho bản xem trước Office."
                );
            }
            ...
            if (activePreviewAbortController === requestController) {
                activePreviewAbortController = null;
            }

            var objectUrl = URL.createObjectURL(pdfBlob);
            showPreviewSource("pdf", objectUrl, file.name, formatSize(file.size), "", true);

            if (pageInput && pageCount > 0) {
    ...
    function showLocalPreview(file, pageInput, previewButton) {
        var kind = previewKindOf(file.name);

        if (kind === "office") {
            showOfficePreview(file, pageInput, previewButton);
            return;
        }
