...
public sealed class PostgreSqlRoutineExecutor : IPhotoCopyHubRoutineExecutor
{
    private readonly IPhotoCopyHubConnectionStringProvider _connectionStringProvider;
    private readonly IPhotoCopyHubRoutineCatalog _routineCatalog;

    public PostgreSqlRoutineExecutor(
        IPhotoCopyHubConnectionStringProvider connectionStringProvider,
        IPhotoCopyHubRoutineCatalog routineCatalog)
    {
        ...
        IReadOnlyList<NpgsqlParameter>? p_arrParameter)
    {
        ...
        {
            throw new InvalidOperationException($"Routine is not registered in PhotoCopyHub whitelist: {p_strRoutineName}");
        }
