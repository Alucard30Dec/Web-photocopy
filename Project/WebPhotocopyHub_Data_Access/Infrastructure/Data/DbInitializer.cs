...
public class DbInitializer : IDbInitializer
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (_configuration.GetValue<bool>("PHOTOCOPYHUB_SEED_ONLY"))
            {
                throw;
            }
        }
    ...
    private async Task EnsureDatabaseReadyAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            await EnsurePhotoCopyHubTablesCreatedAsync(cancellationToken);
        }
        ...
        {
            throw new InvalidOperationException(
                "Không thể khởi tạo PostgreSQL/Supabase. Hãy kiểm tra ConnectionStrings:DefaultConnection hoặc PHOTOCOPYHUB_POSTGRES_CONNECTION, SSL Mode, port, user, password và trạng thái Supabase project.",
                ex);
        }
    }

    private async Task EnsurePhotoCopyHubTablesCreatedAsync(CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex) when (LooksLikeMissingPhotoCopyHubTableException(ex))
        {
            // ChatGPT fix 2026-06-03: Supabase has managed schemas, so EnsureCreated can skip app tables.
            _logger.LogInformation("Supabase database has existing managed tables but PhotoCopyHub tables are missing. Creating PhotoCopyHub tables.");
    ...
    private static bool LooksLikeMissingPhotoCopyHubTableException(Exception exception)
    {
        return ContainsExceptionMatching(exception, item =>
        {
    ...
    {
        var email = GetConfiguredValue("SeedAdmin:Email", "admin@photocopyhub.local");
    ...
    {
        var email = GetConfiguredValue("SeedShopOperator:Email", "operator@photocopyhub.local");
    ...
    private async Task SeedDefaultCustomerAccountsAsync()
    {
        var accounts = new[]
        {
            new
            {
                Email = "sinhvien01@photocopyhub.local",
            ...
            new
            {
                Email = "sinhvien02@photocopyhub.local",
                Password = "Student@123",
            ...
            new
            {
                Email = "sinhvien03@photocopyhub.local",
                Password = "Student@123",
                FullName = "Lê Gia Bảo",
            ...
            new
            {
                Email = "khachhang@photocopyhub.local",
    ...
    private async Task<List<ApplicationUser>> SeedSampleCustomersAsync()
    {
        var samples = new[]
        {
            ...
            new { Email = "khachhang@photocopyhub.local", Password = "Customer@123", FullName = "Khách hàng mặc định", Phone = "0910000099", Address = "Địa chỉ nhận tài liệu sẽ cập nhật khi đặt đơn" }
        };
        ...
        ApplicationUser operatorUser)
    {
        ...
        var sinhVien01 = usersByEmail["sinhvien01@photocopyhub.local"];
        ...
        var sinhVien02 = usersByEmail["sinhvien02@photocopyhub.local"];
        ...
        var sinhVien03 = usersByEmail["sinhvien03@photocopyhub.local"];
        ...
        var khachHang = usersByEmail["khachhang@photocopyhub.local"];
        ...
        ApplicationUser operatorUser)
    {
        ...
        CreateSeedProductOrder(
            user: usersByEmail["sinhvien01@photocopyhub.local"],
        ...
        CreateSeedProductOrder(
            user: usersByEmail["sinhvien03@photocopyhub.local"],
            productsByName: productsByName,
            runningBalancesByUserId: runningBalancesByUserId,
        ...
        ApplicationUser operatorUser)
    {
        ...
        CreateSeedSupportOrder(
            user: usersByEmail["sinhvien02@photocopyhub.local"],
